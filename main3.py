...
def extract_between(text):
    ...
    start_texts = ["AI:","Aarti:"]
    end_texts = ["Human:", "Atharva"]
    start_index = -1
...
def question(input,  personality):
    ...
    response = chain.invoke({"personality": personality ,"name": "Aarti","input": input})
    return response
...
#    ]
    ...
    tab_labels = [f"Girl Assistant" for config in configurations]
    tabs = st.tabs(tab_labels)
    
    for i, tab in enumerate(tabs):
        with tab:
            st.header(f"Girl Assistant")
